...
| # | Decision |
|---|---|
| T1 | **Opt-out — but nothing transmits until the first-run notice has been SHOWN.** AMENDED 2026-08-04 by owner: the notice is a MINIMAL BOTTOM BAR, not a modal — one sentence ("We collect completely anonymous usage data."), an **Opt out** action, a small **Details** link (Preferences → Usage analytics / TELEMETRY.md), and a dismiss; dismissal = keep on (that is what opt-out means). No wall of text — the explanation lives in Preferences and TELEMETRY.md, not the notice. Collection may buffer pre-notice; the network starts only after the notice renders. No silent pre-notice exfiltration, ever. |
...
- **A2 — infra + CLI**: `/v1/telemetry` route + handler (validate → EMF emit →
  ...
  > **DONE — THE CLIENT IS LIT, 2026-08-04** (owner-approved: "opt-out opts them
  > out, but otherwise let's do telemetry"). `TELEMETRY_API_URL` is
  > `https://pcy0z3xjp9.execute-api.us-east-1.amazonaws.com/v1/telemetry`, the
  > flush loop POSTs it (`net.ts postTelemetryBatch` — the ONLY fetch in the
  > directory, pinned to that constant), retires the sent records on a 2xx and
  > stores the batch as `lastBatch`. Every item of the checklist below shipped in
  > that same commit, and `tests/telemetryNet.test.mts` now pins the LIT
  > properties (exact URL, single fetch target, no override, consent gates
  > unchanged). The post-release smoke (`npm run smoke:release`) grew a telemetry
  > leg that proves the whole path against the live stack from a published
  > installer, and reports `LIT-BUT-CLOSED` distinctly when `telemetry_accepting`
  > is off. **`telemetry_accepting` remains the owner's switch and was not
  > flipped by this commit.**
  >
  > The checklist as it was written, for the record — the commit
