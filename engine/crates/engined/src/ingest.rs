...
///
/// EVERY ARM IS A READ OF THE FOLD. A module snapshot, a combat snapshot and a fight search all take
/// `&self` on the sink, and a perf snapshot peeks the meter, so no arm here folds an event, applies a
/// define or moves the ingest's own counters — which is what makes it safe to call at every boundary,
/// including inside the nap. That is a property of the `Ask` enum rather than of this loop: a new arm
/// that needed `&mut` would not compile here, and would belong on the define door instead.
///
