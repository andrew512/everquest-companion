...
import { clampBgAlpha } from '../shared/overlayBgAlpha'
...
export function getOverlayConfig(kind: OverlayKind): OverlayConfig {
  ...
  // …and the background alpha, whose clamp moved OUT of this file with the slider's own numbers
  // (JOS-407, shared/overlayBgAlpha.ts). It is clamped on the way out as well as in for the reason
  // the floor moved at all: the old 0-1 range let a hand-edited store (or a share import's
  // `clamp01`) hold a 0 that no slider could get back off the floor.
  cfg.bgAlpha = clampBgAlpha(cfg.bgAlpha)
