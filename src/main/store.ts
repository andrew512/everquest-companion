...
export function setOverlayConfig(kind: OverlayKind, patch: Partial<OverlayConfig>): OverlayConfig {
  ...
  // The toast's timing blob and the alert lanes' own look, filled for the kind that owns each and
  // dropped everywhere else (./overlayBlobs.ts — its own module because this file is at the
  // factoring ceiling, the uiScale.ts precedent).
