...
//   * AND A SUSPEND TAKES THE SAME DOOR (JOS-307, owner ruling 2026-08-14). A laptop closing its
//     lid under an in-flight request is not an outage and is not a failure of anything — it is the
//     most ordinary event in the life of a desktop app, and 0.27.0 filed twelve of them on its
//     first day. It is warned about exactly like an unreachable network, on the SAME budget (one
//     line per code per session, `MAX_WARNED_UPDATE_CODES` wide), because the two are the same
//     claim: this failure is about the machine, and the second copy adds nothing. What is NOT the
//     same is what happens next — `updater.ts` re-anchors the cadence to the WAKE instead of
//     counting a failure, which is the "retry on resume" half of the ruling.
...
export function logUpdateFailure(
  ...
  if (kind === 'unreachable' || kind === 'suspended') {
    // SOMEBODY ELSE'S NETWORK, OR NOBODY'S AT ALL. One line per code per session, on the console
    // only; the per-check signal that survives is `updateOutcome`'s failureClass, which the caller
    // records either way. The two share a budget on purpose (see the header).
    const code = updateFailureCode(err) ?? kind
    if (takeUnreachableWarning(code)) {
      sinks.warn(
        UPDATER_LOG_PREFIX,
        kind === 'suspended'
          ? `update ${step} was cut short by the machine suspending (${code}); ` +
              'the next check is re-anchored to the wake'
