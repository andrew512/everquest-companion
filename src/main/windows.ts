 ...
 * `overlay:setIdle`, and the whole of the JOS-40 opaque-window rule: an opaque notifier is SHOWN
 * ONLY WHEN IT HAS SOMETHING TO SHOW, because an empty one would be a solid rectangle parked over
 * the game. (Transparent windows are untouched — an empty one is already invisible, and
 * hiding/showing it on every card would be churn for no pixel. `noteNotifierIdle` says which.)
 *
 * IT USED TO BE INFERRED, and that is why it is now stated. The toast's mouse rule
 * (`ignore = !ready ? true : locked ? !hasCards : false`) happens to be true in exactly the
 * states where it draws nothing, so `setOverlayIgnoreMouse` could read idleness off it for free.
 * That coincidence does not survive a second notifier: an ALERT TEXT overlay stays click-through
 * whether or not lines are on screen — deliberately, so it never eats a click meant for the game
 * — so its ignore-state carries no information about what it is drawing. Who owns the mouse and
 * what is on screen are two questions; conflating them was correct for one window and wrong for
 * two. The toast now sends both signals and behaves exactly as it did.
 */
...
export function createOverlayWindow(kind: OverlayKind): void {
  ...
  w.on('ready-to-show', () => {
    ...
    // An OPAQUE notifier opens HIDDEN and is brought up by its own queue (see
    // applyOpaqueNotifierVisibility). Showing it here would put a solid rectangle over the game
    // for the moment between first paint and the renderer's first idle signal — the very thing
