...
//   * the CAVEATS — these are base figures with no crits, focus, AA or resist in them, and no recast
//     (spellMetrics.ts's header states both). The panel says `directional` once, like its neighbour.
//
...
export const COLUMN_TITLE: Record<BestSpellColumn, string> = {
  dps: 'damage over the cast, plus the whole duration for a damage over time spell',
  damage: 'total base damage at this level, every tick included',
  damagePerMana: 'total damage divided by the mana it costs',
  hps: 'healing over the cast, plus the whole duration for a heal over time spell',
