...
//   * CW6 pins the JOS-192 rule: a `/who` row states the loadout AT ITS OWN TIMESTAMP, so a row
//     the evidence in front of it contradicts CUTS a boundary there instead of relabelling the
//     whole slice it happens to land in.
...
import { levelDropBoundaries, whoShiftBoundaries } from '../src/main/modules/comboIntervals'
...
// ---------------------------------------------------------------------------
// CW6 — a /who row states the loadout AT ITS OWN TIMESTAMP (JOS-192).
// ---------------------------------------------------------------------------
...
test('CW6: a /who row that AGREES with the evidence cuts nothing', () => {
  ...
  const covering = snap.intervals.filter(
    (i) => i.startTs <= at('Tue Jul 28 20:42:43 2026') && (i.endTs === null || i.endTs > at('Tue Jul 28 20:16:48 2026'))
  )
...
test('CW6: the detector is silent unless a SUSTAINED class is missing from the row', () => {
  ...
  const obs = (ts: number, candidates: ClassAbbr[], source: ClassObservation['source'] = 'skillUp'): ClassObservation => ({
    ts,
    seq: ts,
    source,
