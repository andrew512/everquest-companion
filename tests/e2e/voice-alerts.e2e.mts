...
async function stepEditor(page: Page): Promise<string> {
  ...
  // relevant". Asserted by walking every value rather than by trusting one, because the failure
  // this guards against is a section that renders for a channel that will never use it.
  // The def arrives as `audio:'speech'` — stepRowPicker set it from the row a moment ago.
  //
  // THE VOICE SIDE IS COUNTED BY ITS MODE PICKER, not by the block around it: the channel selector
  // itself lives inside `alert-speech-block`, so that box is always mounted (it is how you change
  // the channel). What must appear and disappear is the Voice CONTENT — "Say…", the phrase field,
  // the voice override — and `alert-speech-mode` is the first of those.
  const sections = async (): Promise<[number, number]> => [
    await countOf(page, '[data-testid="alert-sound-section"]'),
    await countOf(page, '[data-testid="alert-speech-mode"]')
  ]
  ...
  // The FOURTH channel ('silent', alert-text-overlays D1) is deliberately NOT walked here. It
  // hides both sections — `playsSound`/`speaks` are both false for it, which tests/voiceAlerts and
  // tests/alertPreview pin at the pure seam — but driving this particular option through the MUI
  // menu in the harness is unreliable in a way the other three are not, and a flaky check is
  // worth less than the claim it makes.
