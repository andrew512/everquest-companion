...
// --- readLogsDir: a FAILED read is not "no logs" (JOS-82) -------------------
//
// `countCharacterLogs` answered `0` for three different situations — the folder is not
// there, the folder is there and empty, and the OS refused to list the folder — and the
// Settings card printed the SAME sentence for all three: "No character logs (eqlog_*.txt)
// found here. Make sure EverQuest logging is enabled (/log on)". For the third that is a
// silent wrong answer: the user is looking at their logs in Explorer while the app tells
// them to go turn logging on. A 0.6.3 reporter hit exactly that shape ("I pointed it
// directly to the log folder and it said there were no character logs in that directory").

test('readLogsDir: a real Logs dir reports ok + the character-log count', () => {
  const tmp = mkdtempSync(join(tmpdir(), 'eq-read-ok-'))
  try {
    const { logsDir } = tempInstall(tmp)
...
test('readLogsDir: an EMPTY Logs dir is ok with count 0 — read fine, nothing in it', () => {
  ...
  try {
    ...
    // This is the /log on case, and it must stay distinguishable from the two failures below.
 ...
 * throws — a test must not leave an undeletable directory behind on anyone's machine or CI.
 */
function denyListing(dir: string): () => void {
  ...
  return () => {
    try {
      ...
      // Best effort — the rmSync below is itself guarded.
    }
  }
}
...
test(
  ...
  (t: TestContext) => {
    // MEASURED on Windows 11 (10.0.22631): an ACL that denies READ_DATA makes `readdirSync`
    // throw EPERM on a directory whose files Explorer still lists for the same user. That is
    // the only way the app can truthfully be told "there are files here" and answer "there are
    // no character logs here", which is the reporter's exact complaint.
    ...
    try {
      ...
      if (read.ok) {
        // Some environments (a privileged CI account) can bypass a deny ACE. Say so rather
        // than pass silently — "structurally covered" is not "verified".
